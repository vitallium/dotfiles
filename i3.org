...
** Startup
Set the correct DPI value with xlayoutdisplay. Disabled for now because I set the correct DPI via Xorg configuration.
#+begin_src conf :tangle no
exec --no-startup-id xlayoutdisplay -q
#+end_src

#+begin_src conf :tangle no :file /etc/X11/xorg.conf.d/90-monitor.conf
Section "Monitor"
...
#+begin_src conf :tangle no :file /etc/X11/xorg.conf.d/20-amdgpu.conf
Section "Device"
  Identifier "AMD"
  Driver "amdgpu"
